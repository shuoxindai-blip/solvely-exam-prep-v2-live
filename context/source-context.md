...
Canonical design-system title: Solvely.ai Design System
...
Figma files selected:
- 2026 iOS Solver 迭代【Q2】 (1).fig
- /Users/mac/Desktop/放一下/_26 Solvely Design System Refresh.fig

Locally parsed Figma summaries under `context/figma/`:
- context/figma/2026-ios-solver-q2-1.md
- context/figma/meta.json
- context/figma/archive-inventory.json

Local openable Figma archive:
- context/figma/_26 Solvely Design System Refresh.fig is a symlink to the source file above.
- context/figma/open-source.command opens the source archive in the local Figma app.

Figma MCP status:
- Local archive open confirmed.
- MCP node extraction is not available yet because the local file does not provide a cloud `fileKey` with editor access. A `use_figma` probe returned an edit-access error.

Fonts, logos, and assets selected: no separate files. The Figma archive thumbnail is preserved at `assets/figma-source-thumbnail.png`.

Uploaded brand asset files under `assets/`: `figma-source-thumbnail.png` extracted from the attached Figma archive.
